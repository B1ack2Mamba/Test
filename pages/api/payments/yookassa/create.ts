...
export default async function handler(
    ...
    if (!url || !serviceKey) {
        return res.status(500).json({
            ok: false,
            error: "Server env missing: NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY",
        });
    }
    ...
    if (!Number.isFinite(rub) || rub < 1) return res.status(400).json({ ok: false, error: "amount_rub must be >= 1" });
    if (rub > 500000) return res.status(400).json({ ok: false, error: "amount_rub is too large" });
    ...
    if (!userEmail) {
        return res.status(400).json({ ok: false, error: "User email is missing (needed for receipt)" });
    }
