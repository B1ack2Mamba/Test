...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (status === "queued" || status === "in_progress") {
    return res.status(200).json({ ok: true, done: false, status });
  }
  if (status !== "completed") {
    return res.status(200).json({ ok: true, done: true, status, error: String(json?.error?.message || json?.incomplete_details?.reason || "OpenAI response did not complete") });
  }

  const text = extractOpenAIText(json);
  return res.status(200).json({ ok: true, done: true, status, text });
}
